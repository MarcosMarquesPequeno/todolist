package br.com.marques.todolist.user.utils;

import java.beans.PropertyDescriptor;
import java.util.HashSet;
import java.util.Set;

import org.springframework.beans.BeanUtils;
import org.springframework.beans.BeanWrapper;
import org.springframework.beans.BeanWrapperImpl;

public class Utils {

  public static void copyNonNullProperties(Object source, Object target){
    BeanUtils.copyProperties(source, target, getNullPropetyNames(source));
  }


  public static String[] getNullPropetyNames(Object source){
    //BeanWrapper - interface que fornece acesso as propriedades dentro do java
    //BeanWrapperImpl - implementacao da interface acima
    final BeanWrapper src = new BeanWrapperImpl(source);
    PropertyDescriptor[] pds = src.getPropertyDescriptors();
    
    Set<String> emptyName = new HashSet<>();

    for (PropertyDescriptor pd : pds){
      Object srcValue = src.getPropertyDescriptor(pd.getName());
      if (srcValue == null){
        emptyName.add(pd.getName());
      }
    }

    String[] result = new String[emptyName.size()];
    return emptyName.toArray(result);
    

  }
  
}
